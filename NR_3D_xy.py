...
numSum = 2                         # Eigenvalue sample rate
...
for i in range(0,sampleRate):
    ...
    integral_xy[i] = np.trapz((sigma_xy*1*yEigFun(eta,0)), x=theta)
dbl_int = (np.trapz(integral_xy,x=u))          # Calculation and return of 2nd integral
gammaEigSquared = np.square(a*0*math.pi) + np.square((0+0.5)*math.pi) + np.square(b*0*math.pi)
z_ana = (1 - np.exp(-2 * kpp * height)) / (2 * kpp * height)
A_lmn = ((1 / gammaEigSquared) * dbl_int * z_ana)
print(A_lmn)
            #print("Status", ((ll/(numSum-1))*100)"% complete")
#print("A(000) = ", A_lmn[0,0,0])
#print("A(100) = ", A_lmn[1,0,0])
#print("A(200) = ", A_lmn[2,0,0])

# Function to get A_lmn for respective values of l, m and n
def U(xi, eta, zeta):
    triple_sum_vals = A_lmn*np.sqrt(2)*yEigFun(eta,0)
    triple_sum = triple_sum_vals
