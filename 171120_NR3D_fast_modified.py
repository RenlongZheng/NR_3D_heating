...
spotDiameter = 2.3e-6               # Laser spot diameter (m)
w0 = spotDiameter/2                 # Laser spot radius (m)
sampleRate = 15                     # Spatial sample rate
numSum = 10                          # Eigenvalue sample rate
lambda0 = 514e-9                    # laser wavelength (m)
laser_power = 1.8                    # Laser power in milliwatts
...
print("Radiation pressure = ", Force)

MIR = 1/(1-ref*np.exp(-2*kpp*height))            # Multiple internal reflections
print("Multiple internal reflections multiplier = ", MIR)
...
Q_gen = 0.5*MIR*I0*math.pi*np.square(w0)*(1-np.exp(-2*kpp*height))*(1-np.exp(-u_max))
print("Generated heat", Q_gen)
    ...
    for mm in range(0, numSum):
        for nn in range(0, numSum):
            ...
            for i in range(0,sampleRate):
                sigma_xy = ((0.5*w0**2*length*kpp)/(width*kappa*T0))*MIR*I0*np.exp(-u[i])
...
ax.set_zlabel('$\Delta T$ (K)', fontsize=18, labelpad=10)
...
print("Max delT", np.amax(temp))
print("Avg delT", np.mean(temp))
