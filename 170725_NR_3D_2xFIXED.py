'''
This is a code for calculating the Temperature gradient on the surface of a double clamped beam, irradiated with a Gaussian
...
a = length/width
b = length/height
l = np.linspace(1, numSum, numSum+1)
m = np.linspace(1, numSum, numSum+1)
n = np.linspace(1, numSum, numSum+1)
...
def xEigFun(xi,l):
    if l == 0:
        ...
        return np.sqrt(2)*np.cos((l-1)*math.pi*xi)

def yEigFun(eta,m):
        return np.sqrt(2)*np.sin((m)*math.pi*eta)

def zEigFun(zeta,n):
    if n == 0:
        return 1
    else:
        return np.sqrt(2)*np.cos((n-1)*math.pi*zeta)
...
for ll in range(1, numSum):
    for mm in range(1, numSum):
        for nn in range(1, numSum):
            ...
            gammaEigSquared[ll,mm,nn] = (np.square(a*(ll-1)*math.pi) + np.square((mm)*math.pi) + np.square(b*(nn-1)*math.pi))
            if nn == 0:
