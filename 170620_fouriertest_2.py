...
def sigma_xy(i):
    return ((2*np.square(length)*kpp)/(kappa*Tinf))*I0*np.exp(-u[i])
...
def double_integral(ll,mm):
    ...
    for i in range(0,sampleRate):
        ...
        integral_xy[i] = ((w0**2)/(4*width*length))*np.trapz((sigma_xy(i)*xfunc(xi,ll)*yfunc(eta,mm)), x=theta)    # Calculation of the first integral
...
temp_grad = 5545743.2735
print(40*temp_grad*base_area)
